...
def run_pass_temporal_cleanup(
    ...
    for t in range(1, len(alphas)):
        ...
        if clamp_delta > 0.0:
            prev_for_blend = np.clip(prev, curr - clamp_delta, curr + clamp_delta)
        else:
            prev_for_blend = prev

        blended = curr * (1.0 - ema_map) + prev_for_blend * ema_map
        out = curr.copy()
        out[safe_region] = blended[safe_region]
        ...
        if cfg.temporal_cleanup.edge_snap_enabled:
            ...
            out[edge_band] = np.clip(snap[edge_band], 0.0, 1.0)
